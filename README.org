...
=eglot-multi-preset= provides a mechanism for selecting LSP server presets when starting Eglot. This is particularly useful when working with LSP multiplexers like [[https://github.com/AshleyYakeley/rassumfrassum][rass (rassumfrassum)]] and [[https://github.com/thefrontside/lspx][lspx]] where different projects may require different server combinations.
...
3. Select a preset from the completion menu:
   ...
   - Other options are multi-server presets (e.g., "rass: ty + ruff", "lspx: ty + ruff")
        ...
        '(((python-mode python-ts-mode)
           . (("rass: ty + ruff" . ("rass" "python"))
              ("rass: pyright + ruff" . ("rass" "--" "pyright-langserver" "--stdio"
                                         "--" "ruff" "server"))
              ("lspx: ty + ruff" . ("lspx" "--lsp" "ty server"
                                    "--lsp" "ruff server"))
              ("lspx: pyright + ruff" . ("lspx" "--lsp" "pyright-langserver --stdio"
                                         "--lsp" "ruff server"))))
...
On Windows, Node.js CLI tools installed via npm are provided as =.cmd= batch files (e.g., =rass.cmd=, =lspx.cmd=, =typescript-language-server.cmd=). This package automatically detects Windows and appends the =.cmd= suffix to executable names in the default presets.
...
| lspx: ty + ruff      | ty server + ruff server                      |
| lspx: pyright + ruff | pyright-langserver --stdio + ruff server     |
...
| lspx: ts-ls + eslint            | typescript-language-server --stdio + eslint --stdio |
| lspx: ts-ls + eslint + tailwind | typescript-language-server --stdio + eslint --stdio + tailwindcss --stdio |
...
- [[https://github.com/thefrontside/lspx][lspx]] - LSP multiplexer for running multiple servers in one stream
