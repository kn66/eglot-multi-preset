...
(ert-deftest eglot-multi-preset-track-command-prefix-captures-only-eglot ()
  "Prefix capture should store eglot prefixes and clear stale values otherwise."
  (let ((eglot-multi-preset--pending-prefix-arg 'stale)
        (this-command 'eglot)
        (current-prefix-arg '(4)))
    (eglot-multi-preset--track-command-prefix)
    (should (equal eglot-multi-preset--pending-prefix-arg '(4)))
    (setq this-command 'find-file
...
(ert-deftest eglot-multi-preset-maybe-select-preset-honors-captured-prefix ()
  ...
  (let ((major-mode 'python-mode)
        ...
        (eglot-multi-preset--pending-prefix-arg '(4))
        (prompt nil)
        (orig-args nil))
    (cl-letf (((symbol-function 'eglot-multi-preset--lookup-mode-presets)
               (lambda (_mode)
              ...
              ((symbol-function 'completing-read)
               (lambda (message &rest _args)
                 (setq prompt message)
                 eglot-multi-preset-default-label))
              ((symbol-function 'eglot-multi-preset--apply-workspace-config)
               (lambda (_config) nil)))
      (should (eq (eglot-multi-preset--maybe-select-preset
                   (lambda (&rest args)
                     (setq orig-args args)
                     'ok)
                   'arg1 'arg2)
      ...
      (should (equal orig-args '(arg1 arg2)))
      (should-not eglot-multi-preset--pending-prefix-arg))))
